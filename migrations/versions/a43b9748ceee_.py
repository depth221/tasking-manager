...
def _set_project_counters_from_task_states(project_id: int):
    ...
    tasks_statuses = conn.execute('SELECT task_status, count(task_status) FROM tasks WHERE project_id={0} GROUP BY task_status'''.format(project_id))
    ...
    query = 'UPDATE projects ' + \
            'SET tasks_mapped =' + str(tasks_mapped) + \
            ', tasks_validated=' + str(tasks_validated) + \
            ', tasks_bad_imagery=' + str(tasks_bad_imagery) + \
